 ...
 *
 * Props:
 * - onVerify(token): se llama cuando el usuario completa el checkbox. Recibe
 *   el token que debe enviarse al BackEnd para su validación.
 * - onExpire(): se llama cuando el token expira o el widget falla, para que
 *   el formulario invalide el token que tenía guardado.
 *
 * Expone (via ref):
 * - reset(): reinicia visualmente el widget. Debe llamarse después de un
 *   login/registro fallido, ya que un token de reCAPTCHA es de un solo uso.
 */
const Recaptcha = forwardRef(function Recaptcha({ onVerify, onExpire }, ref) {
  ...
  useEffect(() => {
    ...
    if (!siteKey) {
      ...
      console.error(
        'VITE_RECAPTCHA_SITE_KEY no está configurada. Revisa tu archivo .env.',
      )
      return undefined
    }
